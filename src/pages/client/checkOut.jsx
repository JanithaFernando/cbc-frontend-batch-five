...
export default function CheckoutPage(){
    ...
    async function placeOrder(){
        ...
        try{
        const res=await axios.post(import.meta.env.VITE_BACKEND_URL+"/api/orders",orderInfomation,{
            headers:{
    ...
    return(
        <div className="w-full h-full flex flex-col items-center pt-4 relative">
            <div className="w-[400px] shadow-2xl absolute top-1 right-1 flex flex-col justify-center items-center p-1 gap-10">
                ...
                <button className="text-white bg-accent px-4 py-2 rounded-lg font-bold hover:bg-secondary transition-all duration-300" onClick={placeOrder}>
                    Place Order
                </button>
            </div>
            {
                cart.map(
                    (item,index)=>{
                        return(
                            <div key={item.productId} className="w-[500px] my-4 h-[100px] rounded-tl-3xl rounded-bl-3xl bg-primary shadow-2xl flex flex-row relative justify-center items-center">
                                <img src={item.image} className="w-[100px] h-[100px] object-cover rounded-3xl"/>
                                <div className="w-[250px] h-full flex flex-col jsutify-center items-start pl-4">
                                ...
                                <div  className="max-w-[100px] w-[100px] h-full flex flex-row jsutify-evenly items-center">
